...
IFACEMETHODIMP CDllRegEx::Initialize(PCIDLIST_ABSOLUTE pidlFolder, IDataObject* pdtobj, HKEY hkeyProgID)
{
...
IFACEMETHODIMP CDllRegEx::QueryContextMenu(HMENU hmenu, UINT indexMenu, UINT idCmdFirst, UINT idCmdLast, UINT uFlags)
{
...
IFACEMETHODIMP CDllRegEx::InvokeCommand(LPCMINVOKECOMMANDINFO lpici)
{
	// 1. 基本参数验证
	if (!lpici)
		return E_INVALIDARG;
	...
	if (HIWORD(lpici->lpVerb) != 0)
	{
		...
		{
			if (wcscmp((LPCWSTR)lpici->lpVerb, _T("注册")) == 0)
			{
				RegCommand(CMD_REG);
				return S_OK;
			}
			else if (wcscmp((LPCWSTR)lpici->lpVerb, _T("注销")) == 0)
			{
				RegCommand(CMD_UREG);
				return S_OK;
			}
...
void CDllRegEx::initRegMenu(HMENU hmenu, UINT indexMenu, UINT idCmdFirst)
{
	...
	hmenu_lpmi.dwTypeData = _T("主ICMenu注册dll");
	...
	uhmenu_lpmi0.dwTypeData = _T("ICSubMenu注册dll");
	uhmenu_lpmi1.dwTypeData = _T("ICSubMenu注销dll");
